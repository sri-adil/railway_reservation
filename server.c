...
int login(int sock){
	int type, accnum, fd, valid=1, invalid=0, login_success=0;
	char password[PASS_LENGTH];
	struct account temp;
	read(sock, &type, sizeof(type));
	read(sock, &accnum, sizeof(accnum));
	...
	lock.l_start = (accnum-1)*sizeof(struct account);
	...
	if(type == 1){// User
		...
		lseek(fd, (accnum - 1)*sizeof(struct account), SEEK_CUR);
		...
		lock.l_type = F_RDLCK;
		fcntl(fd,F_SETLK, &lock);
		lseek(fd, (accnum - 1)*sizeof(struct account), SEEK_SET);
		read(fd, &temp, sizeof(struct account));
		if(temp.id == accnum){
			if(!strcmp(temp.pass, password)){
				write(sock, &valid, sizeof(valid));
...
void view_booking(int sock, int id, int type){
	...
	if(fp == 0)
		...
		while(fp>0 && entries<10){
			...
			if(temp.accnum == id && temp.type == type)
				bk[entries++] = temp;
			fp = lseek(fd, -1*sizeof(struct bookings), SEEK_CUR);
		}
		write(sock, &entries, sizeof(entries));
